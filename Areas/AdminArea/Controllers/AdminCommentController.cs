...
namespace Voyager.Areas.AdminArea.Controllers
{
    ...
    public class AdminCommentController : BaseController
    {
        ...
        public IActionResult Details(int id)
        {
            ...
            CommentVM comment = _context.Comments.Include(a => a.Trip).ThenInclude(Trip => Trip.Passenger).Include(a => a.Trip).ThenInclude(Trip => Trip.Driver).Select(q => new CommentVM()
            {
                ID = q.ID,
                TripID = q.TripID,
                Content = q.Content,
